...
export default function MessagesPage() {
  ...
  const conversations = conversationsData as Conversation[] | undefined;
  ...
  const handleSend = () => {
    ...
    // Optimistic Update
    const msgQueryKey = getGetConversationMessagesQueryKey(selectedConvId);
    const previousMessages = queryClient.getQueryData(msgQueryKey);
    ...
    if (previousMessages) {
      queryClient.setQueryData(msgQueryKey, (old: any) => [
        ...(Array.isArray(old) ? old : []),
        {
          id: tempId,
          text,
          sender: { id: user?.id, username: user?.username, displayName: user?.displayName, profilePicture: user?.profilePicture },
    ...
    sendMessage.mutate(
      { data: { recipientId: activeRecipient.id, text } },
      {
        onSuccess: (data: any) => {
          ...
          setShowConvList(false);
          queryClient.invalidateQueries({ queryKey: getGetConversationsQueryKey() });
          queryClient.invalidateQueries({ queryKey: getGetConversationMessagesQueryKey(data.conversationId || selectedConvId) });
        },
        onError: () => {
          // Rollback on error
          if (previousMessages) {
            queryClient.setQueryData(msgQueryKey, previousMessages);
          }
  ...
  const getMessageStatus = (msg: Message) => {
    ...
    const isSent = readCount >= 1;
    
    if (isRead) return <CheckCheck className="w-3.5 h-3.5 text-sky-500" />;
    if (isSent) return <Check className="w-3.5 h-3.5 text-muted-foreground/50" />;
    return null;
  };
          ...
          <ScrollArea className="flex-1">
            {convsLoading ? (
              ...
              <div className="divide-y divide-border/50">
                ...
                {visiblePeople.filter((p) => !(conversations ?? []).some(c => c.participants.some(par => par.id === p.id))).length > 0 && (
                  <div className="px-4 py-2 bg-muted/20 text-[11px] font-bold uppercase tracking-wider text-muted-foreground border-y border-border/30 mt-2">
                    People you follow
                  </div>
                )}
                {visiblePeople
                  ...
                  .map((person) => (
                    <button
                      key={person.id}
                      className={cn("w-full flex items-center gap-3 p-4 text-left hover:bg-muted/30 transition-colors", selectedConvId === person.id && "bg-primary/10")}
        ...
        <div className={cn("flex flex-col flex-1 h-full", !showConvList || selectedConvId ? "flex" : "hidden md:flex")}>
          {selectedConvId && activeRecipient ? (
              ...
              <ScrollArea className="flex-1 p-4">
                {msgsLoading ? (
                    ...
                    {messages.map((msg) => {
                      ...
                      return (
                        <div key={msg.id} className={`flex ${isOwn ? "justify-end" : "justify-start"}`} data-testid="message-item">
                          ...
                          <div className={cn("max-w-xs rounded-2xl px-4 py-2 shadow-sm", isOwn ? "bg-primary text-primary-foreground" : "bg-muted")}>
                            <p className="text-sm break-words leading-relaxed">{msg.text}</p>
                            <div className={cn("mt-1 flex items-center gap-1 opacity-70", isOwn ? "justify-end" : "justify-start")}>
                              <p className="text-[10px]">{timeAgo(msg.createdAt)}</p>
                  ...
                  <div className="flex flex-col items-center justify-center h-full py-10">
                    ...
                    <p className="text-sm text-muted-foreground">Start the conversation with {activeRecipient.displayName}!</p>
                  </div>
                )}
              </ScrollArea>

              <div className="p-4 border-t border-border">
                <div className="flex gap-2 bg-muted/30 p-2 rounded-2xl border border-border/50 focus-within:border-primary/30 transition-colors">
                  <textarea
                    value={messageText}
                    onChange={(e) => setMessageText(e.target.value)}
                    placeholder="Type a message..."
                    className="flex-1 min-h-[44px] max-h-40 resize-none bg-transparent px-3 py-2.5 text-sm outline-none placeholder:text-muted-foreground"
                    onKeyDown={(e) => {
                  ...
                  <Button
                    ...
                    className="rounded-xl h-10 w-10 self-end mb-0.5"
            ...
            <div className="flex flex-col items-center justify-center flex-1 text-center px-4">
              <div className="bg-muted/30 w-24 h-24 rounded-[2.5rem] flex items-center justify-center mb-6">
                <MessageCircle className="w-10 h-10 text-muted-foreground" />
              </div>
              <h2 className="text-2xl font-black tracking-tight mb-2">Your Messages</h2>
              <p className="text-muted-foreground font-medium max-w-xs">
                Select a follower from the list to start a real-time conversation.
              </p>
            </div>
          )}
        </div>
